...
class BEVTransform(BaseModule):
    ...
    Args:
        ...
        top_type (str): Coordinate system in which the task is labeled, 'lidar' or 'ego',
                        eg: 'lidar' for 3D detection and 'ego' for occ
    ...
    def forward(self,
        ...
        Returns:
            ...
            depth: (B*N, D, fH, fW)
        """
        # x = inputs[0]  # (B, N, C_in, fH, fW)  # torch.Size([4, 6, 256, 16, 44])
        B, N, C, H, W = x.shape
        ...
        # return self.view_transform(inputs, depth, tran_feat)

        # ref_3d = self.get_reference_points(self.pc_range[0], self.pc_range[3],
        #                                    self.pc_range[1], self.pc_range[4],
        #                                    self.pc_range[2], self.pc_range[5],
        #                                    self.volume_size[0], self.volume_size[1], self.volume_size[2],
        #                                    bs=B, device=x.device, dtype=x.dtype)

        # post_rots = img_aug_matrix[..., :3, :3]
        # post_trans = img_aug_matrix[..., :3, 3]
        # sensor2ego = camera2ego (occ3d annotate the occupancy in the ego coordinate system)
        # cam2imgs = camera_intrinsics[..., :3, :3]
        # bda = lidar_aug_matrix[..., :3, :3]
        ...
        reference_points_img, volume_mask = self.point_sampling(camera2sensor, camera_intrinsics[..., :3, :3],
                                                                ...
                                                                lidar_aug_matrix)
        # reference_points_img: torch.Size([6, 4, 16*200*200, 1, 2])
        # volume_mask: torch.Size([6, 4, 16*200*200, 1])
        ...
        sampling_feats = einops.rearrange(sampling_feats,
                                          ...
                                          b=bs)
        # TODO: Add more pixels? Like dilation or ASPP. Or, get multi-level feature from here?
        ...
        # TODO: Is there a fundamental difference between z h w and w h z for 3D convolution?
    ...
    # This function must use fp32!!! (det task)
    # This function can use fp16!!! (occ task)
    # TODO: [yz] has no effect here, the reference_points would change to fp16 automatic.
    #  [Done] need self.fp16_enabled = False in __init__()
    @force_fp32()
...
class BEVTransformV2(BaseModule):
    ...
    def forward(self,
        ...
        reference_points_img, volume_mask = self.point_sampling(camera2sensor, camera_intrinsics[..., :3, :3],
                                                                img_aug_matrix[..., :3, :3], img_aug_matrix[..., :3, 3],
                                                                lidar_aug_matrix)
        # reference_points_img: torch.Size([1, 163840, 3])
        # volume_mask: torch.Size([1, 163840, 1])
    ...
    def point_sampling(self, camera2sensor, cam2imgs, post_rots, post_trans, bda, mode='fix'):
        ...
        B, N, _, _ = camera2sensor.shape
        if self.ref_3d.dim() == 2:
            assert self.ref_3d.shape[-1] == 3
            reference_points = self.ref_3d[None, :, :]
        elif self.ref_3d.dim() == 3:
            assert self.ref_3d.shape[-1] == 3 and self.ref_3d.shape[0] == B
            reference_points = self.ref_3d
        else:
            raise NotImplementedError

        num_points = reference_points.shape[1]  # [1/B NP 3]
        # [(NP 3) -> (1 NP 3)] - [(B 3) -> (B 1 3)] -> (B NP 3)

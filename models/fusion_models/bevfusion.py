...
class BEVFusion(Base3DFusionModel):
    def __init__(
        ...
        **kwargs,
        ...
        # if "custom" in kwargs and kwargs["custom"]["type"] == "CrossCoordinateSample":
        #     self.cross_coordinate_sample = build_custom(kwargs["custom"])
        # else:
        #     self.cross_coordinate_sample = None
    ...
    def init_weights(self) -> None:
        # TODO：[yz] only init camera backbone?
        if "camera" in self.encoders:
            self.encoders["camera"]["backbone"].init_weights()
            # self.encoders["camera"]["neck"].init_weights()  # init somewhere else
    ...
    def extract_camera_features(
        ...
        **kwargs,
        ...
        B, N, C, H, W = x.size()  # torch.Size([4, 6, 3, 256, 704])
        x = x.view(B * N, C, H, W)  # torch.Size([24, 3, 256, 704])

        x = self.encoders["camera"]["backbone"](x)  # (torch.Size([24, 512, 32, 88]), torch.Size([24, 1024, 16, 44]), torch.Size([24, 2048, 8, 22]))
        x = self.encoders["camera"]["neck"](x)  # (torch.Size([24, 256, 32, 88]), torch.Size([24, 256, 16, 44]))

        if not isinstance(x, torch.Tensor):
            x = x[0]

        BN, C, H, W = x.size()  # torch.Size([24, 256, 32, 88])
        x = x.view(B, int(BN / B), C, H, W)  # torch.Size([4, 6, 256, 32, 88])
        ...
        return x  # torch.Size([1, 80, 180, 180])

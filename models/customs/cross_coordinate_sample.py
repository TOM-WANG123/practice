...
class CrossCoordinateSample(BaseModule):
    ...
    def __init__(self, point_range: list, point_num: list, lidar_point_range: list, point_type: str = 'ego',
        ...
        """
        super().__init__()
        # ref_ego = self.get_reference_points(-40, 40, -40, 40, 0, 0,
        #                                     200, 200, 1)
        # self.register_buffer('ref_ego', ref_ego)
    ...
    def forward(self, x: torch.Tensor,
        ...
        # TODO: [yz] map reference points from ego coordinate to lidar coordinate
        # TODO: [yz] check effect of complicate transform: (ego -> camera -> lidar) / 6
        #                                             or (ego -> camera -> lidar)[0]
        # p_l = c2l_r @ c2e_r.T @ (p_e - c2e_t) + c2l_t
        # TODO: [yz] check ref_ego would not change the self.ref_points
        # TODO: [yz] check precision difference between transpose and inverse  # [done]: same
        # TODO: [yz] check torch.inverse(bda) = bda.T or use self.ref_3d @ bda replace
        # [(B 3 3) -> (B 1 3 3)] @ [(NP 3) -> (1 NP 3 1)] -> (B NP 3 1)
        # reference_points = torch.inverse(bda.view(B, 1, 3, 3)).matmul(self.ref_3d.view(1, num_points, 3, 1))
        B = lidar_aug_matrix.shape[0]
        num_points = self.ref_points.shape[0]
        if self.point_type == 'ego':
            # TODO: [yz] eliminate redundant operations
            ...
            # lidar data augment
            # ref_lidar = torch.inverse(lidar_aug_matrix[:, :3, :3].view(B, 1, 3, 3)).matmul(self.ref_points.view(1, num_points, 3, 1))
            # ref_lidar = ref_lidar.view(B, num_points, 3, 1).squeeze(-1).unsqueeze(-2)[:, :, :, :2]
            ref_lidar = self.ref_points[:, :2].unsqueeze(-2).unsqueeze(0).repeat(B, 1, 1, 1)

        # xmin, xmax, ymin, ymax = -54.0, 54.0, -54.0, 54.0
        ...
        # x: torch.Size([4, 128, 180, 180])  # bs c h w
        # ref_lidar: torch.Size([4, 40000, 1, 2])  # bs num_points 1 uv/wh
        x = F.grid_sample(
            x,
            ref_lidar,
            mode='bilinear',

...
# from ..utils.tensorrt_utils.trtwrapper import TRTWrapper
...
class DSVT(nn.Module):
    ...
    """
    def __init__(self, input_layer, block_name, set_info, d_model, nhead, dim_feedforward, dropout,
                 activation, output_shape, conv_out_channel, **kwargs):
        super().__init__()
        ...
        self.reduction_type = kwargs.get('reduction_type', 'attention')
        # save GPU memory
        self.use_torch_ckpt = kwargs.get('ues_checkpoint', False)
        ...
        for stage_id in range(stage_num):
            ...
            for i in range(num_blocks_this_stage):
                ...
                norm_list.append(LayerNorm(dmodel_this_stage))
...
class Stage_ReductionAtt_Block(nn.Module):
    def __init__(self, input_channel, pool_volume):
        ...
        self.norm = LayerNorm(input_channel)

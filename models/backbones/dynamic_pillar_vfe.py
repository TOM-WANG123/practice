...
__all__ = ["DynamicPillarVFE", "DynamicPillarVFE3D"]
...
class DynamicPillarVFE(nn.Module):
    def __init__(self, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        super().__init__()
...
class DynamicPillarVFE3D(nn.Module):
    def __init__(self, num_point_features, voxel_size, grid_size, point_cloud_range,
                 use_norm, with_distance, use_absolute_xyz, num_filters, **kwargs):
        super().__init__()
        ...
        self.voxel_x = voxel_size[0]  # 0.3
        self.voxel_y = voxel_size[1]  # 0.3
        self.voxel_z = voxel_size[2]  # 8.0
        self.x_offset = self.voxel_x / 2 + point_cloud_range[0]
        self.y_offset = self.voxel_y / 2 + point_cloud_range[1]
        self.z_offset = self.voxel_z / 2 + point_cloud_range[2]
    ...
    def forward(self, points, **kwargs):
        # points = batch_dict['points']  # (batch_idx, x, y, z, i, e)  # Tensor: (points_num, 6), bs: 2
        ...
        points = points[mask]  # Tensor: (points_num, 6)
        points_coords = points_coords[mask]  # Tensor: (points_num, 3)
        points_xyz = points[:, [1, 2, 3]].contiguous()  # Tensor: (points_num, 3)
        ...
        unq_coords, unq_inv, unq_cnt = torch.unique(merge_coords, return_inverse=True, return_counts=True, dim=0)
        # unq_coords[unq_inv] == merge_coords, 重构原数据
        # unq_coords: Tensor: (unique_num,)
        # unq_inv: Tensor: (points_num,)
        # unq_cnt: Tensor: (unique_num,)
        ...
        if self.with_distance:
            points_dist = torch.norm(points[:, 1:4], 2, dim=1, keepdim=True)
            features.append(points_dist)
        features = torch.cat(features, dim=-1)  # (points_num, 11)

        for pfn in self.pfn_layers:
            features = pfn(features, unq_inv)
        # features: Tensor: (unique_num, 192)
        ...
        voxel_coords = voxel_coords[:, [0, 3, 2, 1]]  # (unique_num, 4)  # [batch_idx, z, y, x]

        # batch_dict['pillar_features'] = batch_dict['voxel_features'] = features  # Tensor: (unique_num, 192)
        # batch_dict['voxel_coords'] = voxel_coords  # (unique_num, 4)  # [batch_idx, z, y, x]

        # return batch_dict
        # return features, voxel_coords
        return {'voxel_features': features, 'voxel_coords': voxel_coords, 'pillar_features': features}

...
class GlobalRotScaleTrans:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.is_train:
            ...
            # TODO: [yz] such a fucking rotation operation! there is a rot_mat_T = rot_mat_T.T in BasePoints.rotate!
            if "points" in data:
                data["points"].rotate(-theta)  # self.tensor[:, :3] @ rot_mat_T.T
                data["points"].translate(translation)
                data["points"].scale(scale)

            gt_boxes = data["gt_bboxes_3d"]
            rotation = rotation @ gt_boxes.rotate(theta).numpy()  # self.tensor[:, :3] @ rot_mat_T
            gt_boxes.translate(translation)
            gt_boxes.scale(scale)
            data["gt_bboxes_3d"] = gt_boxes

            # TODO: [yz] why transpose the rotation?
...
class GridMask:
    ...
    def __call__(self, results):
        # TODO: [yz] check if np.random.rand() return 0?
...
class RandomFlip3D:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        rotation = np.eye(3)
        # data["occ_aug_matrix"] = np.eye(4).astype(np.float32)
        if flip_horizontal:
        ...
        # data["occ_aug_matrix"] = data["occ_aug_matrix"]
        return data
...
class PointsRangeFilter:
    ...
    def __call__(self, data):
        ...
        points = data["points"]
        # if self.to_ego:
        #     # TODO: [yz] add box coord transform here?
        #     l2e_r = torch.from_numpy(data["lidar2ego"][:3, :3])
        #     l2e_t = torch.from_numpy(data["lidar2ego"][:3, 3]).reshape(1, 3)
        #     points.coord = (l2e_r[None, ...] @ points.coord[..., None]).squeeze(-1) + l2e_t
        #     points.tensor[:, :3] = points.coord
...
class CoordinateTransform:
    def __init__(self, to_ego=False):
        """Convert the point cloud under the LiDAR coordinate system to the ego coordinate system.
            For occ task, like Occ3D, the occupancy is annotated in the ego coordinate system. So if we
        use this kind of data to perform occfusion task, we need to transform the point cloud under the
        LiDAR coordinate system to the ego coordinate system.
        Args:
            to_ego (bool): whether to transform points under LiDAR to ego
        """
        self.to_ego = to_ego
